-- Prioritize extract_text jobs over other job types when leasing.
--
-- Without prioritization, a backlog of deep_explain jobs can starve extraction,
-- leaving document_pages/document_extraction_chunks empty and coverage_ratio stuck at 0.
...
BEGIN
  ...
  WHERE pq.id = (
    ...
    ORDER BY
      CASE
        WHEN job_type = 'extract_text' THEN 0
        WHEN job_type = 'deep_explain' THEN 1
        ELSE 2
      END,
      next_run_at ASC,
      created_at ASC
